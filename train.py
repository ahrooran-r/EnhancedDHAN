...
def evaluate(predicted_mask, G2, dataset, device, filename):
    ...
    with torch.no_grad():
        detected_shadow = predicted_mask
...
def train_model( predicted_mask,G2, D1, D2, dataloader, val_dataset, num_epochs, parser, save_model_name='model'):
    ...
    optimizerG = torch.optim.Adam([{'params': G2.parameters()}],
                                  lr=lr,
                                  betas=(beta1, beta2))
    ...
    for epoch in range(num_epochs+1):
        ...
        if(epoch%10 == 0):
            ...
            #G1.eval()
            G2.eval()
            evaluate( predicted_mask, G2, val_dataset, device, '{:s}/val_{:d}'.format('result', epoch))

    return  G2, D1, D2



def main(parser,predicted_mask):
    ...
    G1_update, G2_update, D1_update, D2_update = train_model( predicted_mask,G2, D1, D2, dataloader=train_dataloader,
